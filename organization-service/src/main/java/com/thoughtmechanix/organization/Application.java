...
/**
 * The @EnableResourceServer is used to tell your microservice it’s a protected resource and enforces a filter that
 * intercepts all incoming calls to the service, checks to see if there’s an OAuth2 access token present in the incoming
 * call’s HTTP header, and then calls back to the callback URL defined in the security.oauth2.resource.userInfoUri to
